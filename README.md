...
AI 漫剧/短剧自动生成平台。输入创意灵感，自动完成故事创作→角色设计→剧本→分镜→生图→配音→成片的全流程。
...
|------|------|---------|
| 💡 灵感 | 创意发散，生成故事方向 | LLM |
| 📖 故事 | 完整故事大纲 + 世界观 + 角色 + 力量体系 | LLM + 知识库注入 |
| 📑 章节大纲 | 拆解为多章节结构 + 前情提要 | LLM + 角色状态追踪 |
| 🎨 角色三视图 | 自动生成主要角色正/侧/背三视图，入 OSS | LLM + 图片模型 |
| 📝 剧本 | 创作漫剧剧本（含角色一致性上下文） | LLM + 提示词模板 |
| 🎬 分镜 | 拆解为视觉分镜 + 镜头语言 | LLM |
...
- **核心蓝图** — 故事生成输出完整蓝图：logline、synopsis、style_tags、power_system、golden_finger_detail、world_map_hints、prologue_preview 等
- **提示词模板系统** — 4 个内置模板（故事/章节/剧本/分镜），支持 $variable 替换 + 知识库注入，全阶段上下文串联
...
- **角色三视图** — 从故事角色列表自动同步主要角色，LLM 生成正/侧/背外观描述，图片模型生成三视图，自动入 OSS
...
- **角色上下文注入** — CharacterContextBuilder 将角色卡信息自动注入图片生成 prompt
...
- **知识库管理** — 自定义知识库 CRUD
- **角色卡管理** — 支持编辑/删除，三视图从故事自动同步 + AI 生成
- **存储管理** — S3 / OSS / COS / MinIO 等多云存储配置，支持 SDK 级验证测试
- **Neo4j 图数据库** — 可选，默认 SQLite，切换 `GRAPH_PROVIDER=neo4j`
- **OSS 自动上传** — 全管线制品自动上传到云存储，前端 CDN 加速
- **参数预设** — 保存和复用生成参数
- **项目分享与回收站**
...
- 全自动 8 阶段一键生成（灵感→故事→章节大纲→角色三视图→剧本→分镜→生图→配音→成片）
- 交互式 StoryWizard 6 步故事创作向导
- 角色三视图生成（正/侧/背，LLM 描述 + 图片模型 + OSS 存储）
- 角色卡从故事自动同步，编辑/删除/批量生成三视图
- SSE 流式生成 + 打字机效果
- 多 Provider 路由（Wanx / SiliconFlow / LLM 自动切换）
- AI 模型 Web 后台管理
- 存储供应商配置管理（S3/OSS/COS/MinIO），SDK 级验证测试
- 角色关系图 + 状态追踪（SQLite/Neo4j），自动记忆提取
- 用户角色权限管理（JWT）
- 知识库 + 提示词模板系统（4 个内置模板）
- OSS/CDN 自动加速 — 所有管线制品自动上传 OSS，前端直连 CDN
- 主题切换（深色/浅色/跟随系统）
- 时区自动检测与显示
- 数据分析仪表盘
- 项目分享与回收站
