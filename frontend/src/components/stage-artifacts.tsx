    ...
    <div className="space-y-2">
            ...
            <div key={f.id} className="aspect-[4/5] rounded-xl overflow-hidden bg-zinc-900 ring-1 ring-white/5 hover:ring-violet-500/40 transition-all group relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="absolute top-2 right-2 flex gap-1">
                  <button onClick={(e) => { e.stopPropagation(); setLightbox(src) }} className="p-1.5 rounded-md bg-black/50 text-foreground/80 hover:text-foreground">
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center cursor-zoom-out" onClick={() => setLightbox(null)}>
          ...
          <a href={lightbox} download className="absolute bottom-6 right-6 p-2 rounded-full bg-muted/50 text-white hover:bg-muted/60"><Download className="w-5 h-5" /></a>
          <button onClick={() => setLightbox(null)} className="absolute top-4 right-4 p-2 rounded-full bg-muted/50 text-white"><X className="w-5 h-5" /></button>
        </div>
      )}
    </div>
  )
    ...
    <div className="space-y-3">
        ...
        return (
          <div key={f.id} className="rounded-xl overflow-hidden bg-black ring-1 ring-white/5 group">
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-zinc-900">
              <span className="text-xs text-muted-foreground truncate">{f.file_path.split('/').pop()}</span>
              <div className="flex gap-1">
                ...
                <a href={src} download className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 px-2"><Download className="w-3 h-3" />下载</a>
      ...
      {fullscreen && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center" onClick={() => setFullscreen(null)}>
          ...
          />
          <button onClick={() => setFullscreen(null)} className="absolute top-4 right-4 p-2 rounded-full bg-muted/50 text-white hover:bg-muted/60">
            <X className="w-5 h-5" />
          </button>
        </div>
      )}
  ...
  return (
    <div className="space-y-2">
      ...
      {files.map(f => {
        ...
        return (
          <div key={f.id} className="rounded-xl bg-muted/30 ring-1 ring-white/5 p-3 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                <Music className="w-4 h-4 text-muted-foreground shrink-0" />
                <Badge variant="secondary" className="text-[10px] shrink-0">{typeLabel}</Badge>
                {panelIdx && <span className="text-[10px] text-muted-foreground">{panelIdx}</span>}
              </div>
              <div className="flex gap-1 shrink-0">
                <a href={src} download><Download className="w-3.5 h-3.5 text-muted-foreground hover:text-foreground" /></a>
...
function ScriptGallery({ files, onDelete }: { files: FileRecord[]; onDelete: (id: string) => void }) {
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground uppercase tracking-wider px-1">剧本 · {files.length} 份</p>
...
function ScriptCard({ file, onDelete }: { file: FileRecord; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="rounded-xl bg-muted/30 ring-1 ring-white/5 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="flex gap-1 shrink-0">
          ...
          {content && (
            <button onClick={async () => { await navigator.clipboard.writeText(content); setCopied(true); setTimeout(() => setCopied(false), 2000) }} className="text-xs text-muted-foreground hover:text-foreground px-1">
      ...
      {!collapsed && loading && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground"><Loader2 className="w-3 h-3 animate-spin" />加载中...</div>
      )}
      {!collapsed && !loading && content && (
        <pre className="text-xs bg-black/30 rounded-lg p-3 overflow-auto max-h-72 whitespace-pre-wrap font-mono leading-relaxed text-muted-foreground">
          {content}
        </pre>
      )}
    </div>
...
function StoryboardGallery({ files, onDelete }: { files: FileRecord[]; onDelete: (id: string) => void }) {
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground uppercase tracking-wider px-1">分镜 · {files.length} 份</p>
      ...
      {!collapsed && data?.panels && (
        <div className="space-y-2 max-h-96 overflow-auto">
          {data.panels.map((panel: Record<string, unknown>, i: number) => (
            <div key={i} className="border border-border rounded-lg p-2 text-xs space-y-1 bg-black/20">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-[10px] px-1 py-0 border-border/80">Panel {i + 1}</Badge>
                {Boolean(panel.emotion) && <span className="text-muted-foreground italic">{String(panel.emotion)}</span>}
              </div>
              {Boolean(panel.scene_description) && <p className="leading-relaxed text-muted-foreground">{String(panel.scene_description).slice(0, 250)}</p>}
              {Boolean(panel.camera_angle) && <p className="text-[10px] text-muted-foreground">🎬 {String(panel.camera_angle)}</p>}
            </div>
          ))}
        </div>
      )}
...
function TextGallery({ files, onDelete }: { files: FileRecord[]; onDelete: (id: string) => void }) {
  return (
    <div className="space-y-3">
      <p className="text-[11px] text-muted-foreground uppercase tracking-wider px-1">生成文件 · {files.length} 份</p>
      {files.map(f => <ScriptCard key={f.id} file={f} onDelete={onDelete} />)}
    </div>
  )
}
